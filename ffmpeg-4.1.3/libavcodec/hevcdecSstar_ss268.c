...
static int ss_hevc_vdec_init(AVCodecContext *avctx)
{
    ...
    if (!(avctx->flags & (1 << 17))) {
        STCHECKRESULT(MI_VDEC_SetOutputPortLayoutMode(VdecDev, E_MI_VDEC_OUTBUF_LAYOUT_LINEAR));
    } else {
        av_log(avctx, AV_LOG_WARNING, "hevc enable vdec tilemode\n");
        STCHECKRESULT(MI_VDEC_SetOutputPortLayoutMode(VdecDev, E_MI_VDEC_OUTBUF_LAYOUT_TILE));
    }
    ...
    STCHECKRESULT(MI_VDEC_SetOutputPortAttr(VdecDev, stVdecChn, &stOutputPortAttr));

    av_log(NULL, AV_LOG_INFO, "sshevc video w/h = [%d,%d], vdec output w/h = [%d,%d]\n", avctx->width, avctx->height, stWidth, stHeight);
...
static int ss_hevc_send_stream(SsHevcContext *ssctx, uint8_t *data, int size, int64_t pts, int flag)
{
    ...
    s32Ret = MI_VDEC_SendStream(VdecDev, stVdecChn, &stVdecStream, 30);
...
static av_cold int ss_hevc_decode_free(AVCodecContext *avctx)
{
    ...
    STCHECKRESULT(MI_VDEC_StopChn(0, stVdecChn));
    STCHECKRESULT(MI_VDEC_DestroyChn(0, stVdecChn));
                                      ...
                                      int *got_frame, AVPacket *avpkt)
{
    ...
    else
    {
        *got_frame = 0;
        ...
        if (ret == MI_ERR_VDEC_FAILED && (!(*got_frame) || avctx->flags & (1 << 8))) {
...
static int ss_hevc_receive_frame(AVCodecContext *avctx, AVFrame *frame)
{
    ...
    while (!frame->buf[0])
    {
        ...
        if (!avpkt->data && !avci->draining)
        {
            ...
            if (ret1 == MI_ERR_VDEC_FAILED && (!got_frame || avctx->flags & (1 << 8))) {
                return MI_ERR_VDEC_FAILED;
            }
        }
...
static void ss_hevc_decode_flush(AVCodecContext *avctx)
{
    if (avctx->flags & (1 << 7))
    {
        MI_VDEC_FlushChn(0, VDEC_CHN_ID);
        //av_log(avctx, AV_LOG_INFO, "ss_hevc_decode_flush done!\n");
    }
}

AVCodec ff_sshevc_ss268_decoder = {
    .name                  = "sshevc_ss268",
