    ...
    while(1)
    {
        ...
        if (is->p_audio_frm != NULL)
        {
            ...
            do {
                ...
                do{
                    s32RetSendStatus = MI_AO_SendFrame(AUDIO_DEV, AoChn, &stAoSendFrame, 128);
...
static int open_audio_playing(void *arg)
{
    ...
    ret = sstar_audio_init(AUDIO_DEV);
    if (ret < 0) {
        return -1;
    }
