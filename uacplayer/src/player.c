...
static void audio_decoder_abort(player_stat_t *is)
{
    ...
    if (is->audio_uac_output_tid)
        pthread_join(is->audio_uac_output_tid, NULL);
    printf("2.1audio_uac_output_tid pthread_join exit!\n");
	
    if (is->audio_recv_tid)
        pthread_join(is->audio_recv_tid, NULL);
    printf("2.2audio_recv_tid pthread_join exit!\n");

    if (is->audio_play_tid)
        pthread_join(is->audio_play_tid, NULL);
    printf("2.3audio_play_tid pthread_join exit!\n");
...
player_stat_t *player_init(const char *p_input_file)
{
    ...
    if ((frame_queue_init(&is->video_frm_queue, &is->video_pkt_queue, VIDEO_PICTURE_QUEUE_SIZE, 1) < 0 ||
		...
		frame_queue_init(&is->audio_recv_frm_queue, &is->audio_pkt_queue, SAMPLE_QUEUE_SIZE, 1) < 0)
    {
        goto fail;
    }
