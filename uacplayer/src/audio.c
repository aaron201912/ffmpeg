...
#ifdef UAC_SUPPORT
static void * audio_uac_output_thread(void *arg)
{
    player_stat_t *is = (player_stat_t *)arg;
    int audio_size = 0;
	int remain_audio_size = 0;
    int pause = 0;
    int last_pause =0;
    ...
    while(1)
    {
        if(is->abort_request) 
        {
            printf("audio uac output thread exit\n");
            break;
        }		
        //av_usleep(10 * 1000);
        ...
        else
        {
            ...
            if (is->p_audio_frm != NULL)
            {
                ...
                {					
					...
					{			
						int ret = 0;
						Audio_Handle_t *audio_handle = &g_audio_handle;
						Audio_Buffer_t audio_buf;
						memset(&audio_buf, 0, sizeof(Audio_Buffer_t));
						ret = audio_get_buf(audio_handle, &audio_buf, AS_OUT_MODE);
						if(ret == 0)
						{
							if(g_buf_handle_mode == BUF_HANDLE_MODE_FFMPEG)
							{								
								if((audio_size + remain_audio_size)>= audio_buf.length)
								{
									if(remain_audio_size == 0)
									{
										memcpy(audio_buf.buf,(void *)(&(is->p_audio_frm[DataIndex])), audio_buf.length);										
										audio_size -= audio_buf.length;
										DataIndex += audio_buf.length;
									}
									else
									{
										memcpy(&data[remain_audio_size],(void *)(&(is->p_audio_frm[DataIndex])), audio_buf.length - remain_audio_size);
										audio_size -= audio_buf.length - remain_audio_size;
										DataIndex += audio_buf.length - remain_audio_size;
										remain_audio_size = 0;
										memcpy(audio_buf.buf,data, audio_buf.length);
									}
								}	
								else
								{
									//memcpy(audio_buf.buf,(void *)(&(is->p_audio_frm[DataIndex])), audio_size);
									memcpy(data,(void *)(&(is->p_audio_frm[DataIndex])), audio_size);
									remain_audio_size = audio_size;
									DataIndex = 0;
									audio_size = 0;
								}	
							...
							if(ret != 0){printf("audio_put_buf fail %d\n",ret);
								break;}
							
							is->audio_write_buf_size = audio_buf.length;
	...
	free(data);
    printf("audio_uac_output_thread exit\n");
    return NULL;
}
static void * audio_receive_thread(void *arg)
{
	uint8_t * data_test = malloc(4096);
	//uint8_t * data_test1 = malloc(4096);
    player_stat_t *is = (player_stat_t *)arg;
    AVFrame *p_frame = av_frame_alloc();
    frame_t *af;
	void *ai_frame;
    ...
    {
        if(is->abort_request) {
            printf("audio receive thread exit\n");
            break;
        }


		if (is->functions.audio_dump_ai_buf){
			
			int ret = 0;
			p_frame->data[0] = data_test;
			ret = is->functions.audio_dump_ai_buf(is, (uint8_t *)(p_frame->data[0]), &(is->audio_recv_frm_size),ai_frame);
		}

		frame_queue_t *f = &is->audio_recv_frm_queue;		
		af = &f->queue[f->windex];
		av_frame_move_ref(af->frame, p_frame);
					
		// 更新音频frame队列大小及写指针
		frame_queue_push(&is->audio_recv_frm_queue);
		av_frame_unref(p_frame);

		if (is->functions.audio_release_ai_buf)
		{	
			if(is->audio_recv_frm_size)
				is->functions.audio_release_ai_buf(is, ai_frame);
		}
        ...
        while (f->size >= f->max_size && !f->pktq->abort_request) {
	        ...
	        if(is->abort_request) {
	            printf("audio receive thread exit 1\n");
	            break;
	        }
			printf("pthread_cond_wait 1\n");
            pthread_cond_wait(&f->cond, &f->mutex);
            if (is->seek_flags & (1 << 5)) {
				printf("pthread_cond_wait exit while\n");
                break;
            }
...
the_end:
    ...
    //printf("audio_receive_thread exit\n");
    return NULL;
}
...
static void * audio_playing_thread(void *arg)
{
    ...
    while(1)
    {
		...
		while (f->size - f->rindex_shown <= 0 && !f->pktq->abort_request) 
		{
			//printf("audio_playing_thread runing 1  ##(%d) - (%d)## (%d)\n", f->size , f->rindex_shown,f->pktq->abort_request);
			if (!is->audio_complete && is->eof && !is->audio_pkt_queue.nb_packets)
			{
				is->audio_complete = 1;
				if (is->video_complete && is->audio_complete) {
				...
				av_log(NULL, AV_LOG_INFO, "audio play completely %d  !\n", is->video_complete);
			}
			gettimeofday(&now, NULL);
			outtime.tv_sec	= now.tv_sec + 1;
			outtime.tv_nsec = now.tv_usec * 1000;
		...
		//printf("===>audio_playing_thread frame_queue_next now(r_idx: %d, r_idx_show: %d)(%d)\n", f->rindex, f->rindex_shown,(f->rindex + f->rindex_shown) % f->max_size);
		af = &f->queue[(f->rindex + f->rindex_shown) % f->max_size];

		frame_queue_next(&is->audio_recv_frm_queue);
		
        is->p_audio_recv_frm = af->frame->data[0];

        {
            if (is->p_audio_recv_frm != NULL)
            {
                //printf("save audio len: %d\n",is->audio_frm_size);
                if (is->functions.audio_play)
                {
                    is->audio_write_buf_size = is->functions.audio_play(is, (void *)is->p_audio_recv_frm, is->audio_recv_frm_size);
                    audio_has_data = true;
                }
                //printf("audio frame pts: %f\n", get_clock(&is->audio_clk));
            }
...
static int open_audio_playing(void *arg)
{
    ...
    is->audio_cp_index = 0;
	#ifdef UAC_SUPPORT
    is->audio_recv_frm_size = 0;	
    prctl(PR_SET_NAME, "audio_uac_output");
    ret = pthread_create(&is->audio_uac_output_tid, NULL, audio_uac_output_thread, (void *)is);
    if (ret != 0) {
        av_log(NULL, AV_LOG_ERROR, "audio_uac_output_thread create failed!\n");
        is->audio_uac_output_tid = 0;
        return -1;
    }

    prctl(PR_SET_NAME, "audio_recv");
    ret = pthread_create(&is->audio_recv_tid, NULL, audio_receive_thread, (void *)is);
    if (ret != 0) {
    ...
    prctl(PR_SET_NAME, "audio_playing");
...
int open_audio(player_stat_t *is)
{
    int ret;
#ifdef UAC_SUPPORT
	unsigned int g_audio_index = is->uac_device;	
	char path[20] = {};
	sprintf(path, "/dev/snd/pcmC%dD0", g_audio_index);
	printf("device: %s\n", path);
	memcpy(g_audio_handle.path, path, 20);
	g_audio_handle.card = g_audio_index;
	g_audio_handle.device = 0;
	g_buf_handle_mode = is->buf_handle_mode;

	if(audio_init(&g_audio_handle, AS_OUT_MODE) != 0)
	{
