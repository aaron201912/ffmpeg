...
#ifdef UAC_SUPPORT
	int (*audio_dump_ai_buf)(void *args, void *data, int *len,  void *ai_frame);
	int (*audio_release_ai_buf)(void *args, void *ai_frame);
#endif
} player_func_t;
...
#ifdef UAC_SUPPORT
    frame_queue_t audio_recv_frm_queue;
#endif
...
#ifdef UAC_SUPPORT
    uint8_t *p_audio_recv_frm;               // 指向待播放的一帧音频数据，指向的数据区将被拷入SDL音频缓冲区。若经过重采样则指向audio_frm_rwr，否则指向frame中的音频
#endif
    uint8_t *audio_frm_rwr;             // 音频重采样的输出缓冲区
    unsigned int audio_frm_size;        // 待播放的一帧音频数据(audio_buf指向)的大小
#ifdef UAC_SUPPORT
    unsigned int audio_recv_frm_size;        // 待播放的一帧音频数据(audio_buf指向)的大小
#endif
...
#ifdef UAC_SUPPORT
    pthread_t audio_recv_tid;    //audio播放线程		
    pthread_t audio_uac_output_tid;    //audio播放线程
#endif
